...
# Use bash installed by homebrew
...
# Function to perform combined Google and ChatGPT search
sgg() {
    if [ -z "$1" ]; then
        echo "Usage: sgg <search term>"
        return 1
    fi
    ...
    # Execute ChatGPT search
    chatgpt "$search_term"
}
 
# Function to perform combined Google and Youtube search
sgy() {
    if [ -z "$1" ]; then
        echo "Usage: sgy <search term>"
        return 1
    fi
